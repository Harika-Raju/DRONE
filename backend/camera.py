...
def send_udp_coordinate(latitude, longitude, altitude, host="127.0.0.1", port=9999):
    try:
        ...
        sock.close()
        print(f"[UDP] Sent to {host}:{port} -> {payload}")
...
def run_video_detection_with_distance(weights_path, altitude, initial_latitude, initial_longitude, output_dir="output", conf_threshold=0.25):
    ...
    while True:
        ...
        if len(detections) > 0:  # If there are any detections
            ...
            # --- Added: send over UDP to backend ---
            send_udp_coordinate(new_latitude, new_longitude, altitude, host="127.0.0.1", port=9999)
